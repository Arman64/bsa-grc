...
function getDocs() {
  return {
    ...
    description: "BSA GRC Blog Automation via MCP / n8n / AI Agent - Neon DB",
    auth: {
      required: true,
    ...
    storage: "Neon Postgres - data persist di Vercel, tidak hilang seperti JSON",
  };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Artikel berhasil dipublish otomatis via MCP ke Neon DB",
        data: {
          id: newBlog.id,
          slug: newBlog.slug,
          title: newBlog.title,
