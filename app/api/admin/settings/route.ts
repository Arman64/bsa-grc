...
export async function GET() {
 try {
  ...
  console.error(e);
  return NextResponse.json({ success: false, message: "Gagal ambil pengaturan" }, { status: 500 });
 }
}
...
export async function PUT(request: NextRequest) {
 try {
  ...
  return NextResponse.json({ success: true, message: "Pengaturan berhasil diupdate di", data: body });
 } catch (error) {
  console.error(error);
