...
export async function GET() {
  try {
    ...
    console.error(e);
    return NextResponse.json({ success: false, message: "Gagal ambil layanan, cek DATABASE_URL Neon" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: "Layanan berhasil diupdate di Neon DB", data: updated });
  } catch (error) {
    console.error(error);
