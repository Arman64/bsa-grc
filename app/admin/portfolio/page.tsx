...
import { Plus, Edit3, Trash2, Upload, Search, X, Save, MapPin } from "lucide-react";
...
export default function AdminPortfolioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (json.success) {
      setShowModal(false);
      setEditing(null);
      setForm({ title: "", category: "Kubah GRC", location: "", year: new Date().getFullYear().toString(), image: "", diameter: "", client: "", description: "" });
      fetchData();
    } else {
      alert(json.message);
    }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Upload gambar, atur teks, kategori, lokasi proyek. Max file 5MB JPG/WebP.</p>
          </div>
          <button
            onClick={() => { setEditing(null); setForm({ title: "", category: "Kubah GRC", location: "", year: new Date().getFullYear().toString(), image: "", diameter: "", client: "", description: "" }); setShowModal(true); }}
        ...
        <div className="bg-white rounded-2xl border shadow-soft p-4 flex flex-col sm:flex-row gap-3 justify-between">
          ...
          <p className="text-sm text-muted-foreground flex items-center">
            Total: <span className="font-bold text-foreground ml-1">{filtered.length} proyek</span>
          </p>
        </div>
              ...
              <div key={item.id} className="bg-white rounded-2xl border shadow-soft overflow-hidden group hover:shadow-large transition-all">
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    ...
                    <span className="text-xs font-medium text-gold-700">{item.diameter || "Ø 6m"}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[1.5rem] shadow-large w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b p-6 flex justify-between items-center rounded-t-[1.5rem]">
                <h3 className="font-bold text-lg">{editing ? "Edit Portofolio" : "Tambah Portofolio Baru"}</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div>
                  <label className="text-sm font-semibold mb-2 block">Gambar Proyek * (Upload)</label>
                  <div className="border-2 border-dashed rounded-xl p-4 text-center bg-muted/30 hover:bg-muted/50 transition-colors">
                    {form.image ? (
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium">Klik untuk upload gambar (JPG/WebP max 5MB)</p>
                        <p className="text-xs text-muted-foreground mt-1">Atau masukkan URL manual di bawah</p>
                      </div>
                    )}
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button type="submit" className="flex-1 bg-maroon-700 text-white px-6 py-3 rounded-xl font-bold hover:bg-maroon-800 flex items-center justify-center gap-2">
                    <Save className="w-4 h-4" /> {editing ? "Update" : "Simpan"} Portofolio
                  </button>
                </div>
              </form>
            </div>
