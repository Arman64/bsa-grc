...
export default async function TestimonialSection() {
  ...
  let header: any = HOME_DEFAULT.testimonialsHeader;
  try {
    const [dbTestimonials, content] = await Promise.all([getTestimonialsData(), getPageContentCached("beranda")]);
    testimonials = dbTestimonials;
    header = content.sections.testimonialsHeader || header;
  } catch {}
  ...
  return (
    <section className="cv-auto py-16 lg:py-24 bg-white relative overflow-hidden">
      <div className="container mx-auto px-4 lg:px-8">
        <SectionHeader
          badge={`${header.badge} - ${testimonials.length} Ulasan`}
          badgeVariant="gold"
          title={header.title}
