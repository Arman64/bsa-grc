...
export default async function ServicesSection() {
  let services: any[] = [];
  let header: any = HOME_DEFAULT.servicesHeader;
  let cta: any = HOME_DEFAULT.servicesCta;
  let whatsapp = "6281230469914";
  try {
    const [dynamic, content, settings] = await Promise.all([
      getServicesData(),
      getPageContentCached("beranda"),
      getSettingsCached(),
    ]);
    if (dynamic.length > 0) services = dynamic as any;
    header = content.sections.servicesHeader || header;
    cta = content.sections.servicesCta || cta;
    whatsapp = settings?.company?.whatsapp || whatsapp;
  } catch {}

  const whatsappLink = waLink(whatsapp);

  if (services.length === 0) {
    return (
      <section className="cv-auto py-16 lg:py-24 bg-white">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <SectionHeader badge={header.badge} badgeVariant="gold" title={header.title} description="Database layanan kosong. Silakan tambah layanan melalui halaman admin." />
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="cv-auto py-16 lg:py-24 bg-white relative overflow-hidden">
      ...
      <div className="relative container mx-auto px-4 lg:px-8">
        <SectionHeader badge={header.badge} badgeVariant="gold" title={header.title} description={header.description} />
        ...
        <div className="mt-12 lg:mt-16 grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            return (
              ...
              >
                ...
                <div className="relative h-44 rounded-xl overflow-hidden bg-gradient-to-br from-gold-50 via-white to-maroon-50/30 mb-5 border border-border group-hover:border-gold-100 transition-colors">
                  <Image
                    src={service.originalImage || service.image}
                ...
                <p className="text-sm text-muted-foreground leading-relaxed mb-4 flex-1 line-clamp-2">{service.description}</p>
          ...
          <div className="card-portfolio p-7 bg-gradient-to-br from-maroon-700 via-maroon-700 to-maroon-900 text-white flex flex-col justify-between relative overflow-hidden rounded-2xl shadow-maroon border-0 min-h-[380px]">
            ...
            <div className="relative">
              ...
              <h3 className="font-bold text-[22px] leading-tight mb-3">{cta.title}</h3>
              <p className="text-sm text-white/80 leading-relaxed mb-6">{cta.description}</p>

              <div className="space-y-3 mb-6">
                {(cta.bullets || []).map((item: string) => (
            ...
            <div className="relative space-y-3">
              ...
              <p className="text-[11px] text-white/60 text-center">{cta.note}</p>
            </div>
          </div>
        </div>
      </div>
