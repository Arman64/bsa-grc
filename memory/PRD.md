...
## Landing Page admin bug fix (2026-08)
- User reported `/admin/services` Landing Page editor was missing several sections that the public `/layanan/[slug]` page (`components/sections/LandingPageService.tsx`) actually renders — data existed in DB (seeded) but had NO admin UI, so it couldn't be edited. Added full CRUD UI for: Pain Points (5), Benefits/Keunggulan grid (6), Social Proof rating/reviews/projects/provinces + repeatable Testimonials with photo upload (7), FAQ (10, renumbered from old placeholder note). Also added hero gallery-strip image management (add/remove) which existed in schema but had no visible management UI. Renumbered CTA→8, Risk Reversal→9 for consistency; removed the old "diedit via file JSON langsung" placeholder note.
- Verified via screenshots (all 4 new sections render with real seeded data for "Kubah GRC" service) + curl round-trip PUT test (add/remove test FAQ entry, confirmed persists correctly through generic `updateService`).

## Security Audit + fixes (2026-08)
User requested a full security audit (`security_audit_agent`). Verdict was FAIL initially due to 1 HIGH + 2 MEDIUM confirmed findings; all fixed and verified by `testing_agent` across iterations 6-7 (23/23 tests passed):
- **[HIGH] Stored XSS in blog content** - `marked`-rendered HTML had no sanitizer; an MCP `blog:write`/`blog:edit` token holder could embed `<script>`/`onerror` in article content. Fixed: `lib/markdown.ts` now runs final HTML through `sanitize-html` (strict tag/attribute allowlist, blocks all `on*` handlers and disallowed schemes like `javascript:`).
- **[HIGH, found during regression] Stored XSS via JSON-LD structured data** - `JSON.stringify(schema)` inside `dangerouslySetInnerHTML` for `<script type="application/ld+json">` doesn't escape `</`, so a blog excerpt/title/keywords containing `</script><img onerror=...>` breaks out of the tag. Fixed across all 5 emitters (`app/[slug]/page.tsx`, `app/layout.tsx`, `app/page.tsx`, `app/layanan/[slug]/page.tsx`, `app/portofolio/[id]/page.tsx`) via new `safeJsonLd()` helper in `lib/seo.ts` (escapes `<`,`>`,`&`,U+2028,U+2029).
- **[MEDIUM] MCP tokens stored/returned in plaintext** - `bsa_api_tokens.token` now stores a SHA-256 hash only; raw secret is returned ONCE at creation (`createToken()`), `listTokens()` returns a non-reversible `tokenPrefix` instead, `validateToken()` hashes+constant-time-compares. Migration (`scripts/migrate-tokens.mjs`, already run) added `token_prefix` column and hashed the 1 pre-existing real token ("hermes", actively used) in place - fully backward compatible for its external client. Admin UI (`/admin/mcp`) updated: listed tokens show masked prefix only (no reveal/copy), the one-time creation banner still shows the full raw token with a "won't be shown again" warning.
- **[MEDIUM] SSRF in WP-import image downloader** - `lib/storage.ts downloadAndStoreImage()` now DNS-resolves the hostname and rejects private/loopback/link-local/metadata IP ranges before fetching, and uses `redirect:"manual"` (rejects redirect-based bypasses). Only affects the WordPress-XML-import external URL fetcher, not the direct upload endpoint.
- **[P3 hardening]** `lib/jwt.ts` no longer falls back to a hardcoded secret (fails closed if `JWT_SECRET`/`ADMIN_SESSION_SECRET` unset); `app/api/media/[...path]/route.ts` now 404s any path not under the `bsa-grc/` prefix.
- **Bonus fix (found while testing)**: pre-existing unrelated bug in `lib/markdown.ts` link renderer crashed on ANY markdown link (`[text](url)`), silently falling back to unstyled plain text - fixed by not calling the buggy internal `marked` renderer method.
